...
<template>
    <div v-if="list.length > 0" class="p-4">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            <div v-for="(item, index) in list" :key="index" class="relative" @mousemove="st.pIndex = index"
                    ...
                    <template v-if="item.artifacts && item.artifacts.length > 0 && item.artifacts[0].url">
                        <div v-if="mapRef.has(item.id)">
                            <NButton size="small" type="primary" @click="reRunwayFeed(item.id)">
                                {{ $t('video.repeat2') }}</NButton>
                        </div>
                        <video v-else loop playsinline :controls="st.pIndex == index" referrerpolicy="no-referrer"
                            :poster="item.artifacts[0].previewUrls[0]" class="w-full h-full object-cover"
                            @error="videoError(item, index)">
                            <source :src="item.artifacts[0].url" referrerpolicy="no-referrer" type="video/mp4"
                ...
                <div class="flex justify-between items-center">
                        ...
                        <n-popover trigger="hover">
                            <template #trigger>
                                <div class="line-clamp-1">
                                    ...
                                    <template v-else>{{
                                        item.options.gen2Options?.text_prompt ? item.options.gen2Options.text_prompt:
                                        item.name }}</template>
                            ...
                            <div class=" max-w-[300px]" v-if="item.options.text_prompt">{{ item.options.text_prompt }}
                            </div>
                    ...
                    <div class="flex justify-end items-center pt-1"
                        v-if="item.artifacts && item.artifacts.length > 0 && item.artifacts[0].url">
