...
<template>
    <div class="p-2">
        ...
        <div>
            <n-input v-model:value="luma.user_prompt" :placeholder="$t('video.descpls')" type="textarea" size="small"
        ...
        <div v-if="exLuma" class="pt-1">
            ...
            <div
                class="relative flex items-center justify-center bg-white bg-opacity-10 rounded-[5px] overflow-hidden aspect-[16/8.85] ">
                <video v-if="exLuma.video?.url || exLuma.video?.download_url"
                    :src="exLuma.video?.download_url ? exLuma.video?.download_url : exLuma.video?.url"
                    @error="$event.target.src = exLuma.video?.url" loop playsinline controls
        ...
        <div class="pt-1">
            <div class="flex justify-start  items-end">
                <div>
                    ...
                    <div class="h-[80px] w-[80px] overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
                        @click=" fsRef.click()">
                ...
                <div class="pl-2">
                    <input type="file" @change="selectFile2" ref="fsRef2" style="display: none"
                        accept="image/jpeg, image/jpg, image/png, image/gif" />
                    <div class="h-[80px] w-[80px] overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
                        @click=" fsRef2.click()">
        ...
        <div class="pt-1">
            <div class="flex justify-between items-end">
                <div class="pb-1 text-right">
                    <NTag v-if="exLuma || luma.user_prompt != '' || luma.image_url != '' || luma.image_end_url != ''"
                        type="success" size="small" round><span class="cursor-pointer"
                            @click="clearInput()">{{ $t('video.clear') }}</span>
                    </NTag>
                </div>
                <div>
                    <NButton :loading="st.isDo" type="primary" :disabled="!canPost" @click="generate()">
