...
<template>
    <div>
        <div v-if="st.isLoadImg">
            <NImage v-if="chat.opt?.imageUrl" :src="st.uri_base64 ? st.uri_base64 : chat.opt.imageUrl"
        ...
        <div v-else-if="chat.opt?.imageUrl" class="w-[200px] h-[150px] flex flex-col justify-center items-center">
            ...
            <NButton type="primary"><a :href="chat.opt?.imageUrl" target="_blank">{{ $t('mjchat.openurl') }}</a>
            </NButton>
        </div>
    </div>
</template>