...
<template>
    <div v-if="st.isLoadImg">
        ...
        <template v-else-if="chat.opt?.progress">
            ...
            <div v-else-if="chat.opt?.action != 'IMAGINE'" class="py-2 text-[#666]  whitespace-pre-wrap">{{
                chat.opt?.promptEn }} (<span v-html="chat.opt?.action"></span>)</div>
            <NImage v-if="chat.opt.imageUrl" :src="st.uri_base64 ? st.uri_base64 : mjImgUrl(chat.opt.imageUrl)"
                class=" rounded-sm " :class="[isMobile ? '' : '!max-w-[500px]']" />
            <div v-if="chat.opt?.status == 'SUCCESS'" class=" space-y-2">
                    ...
                    <div v-for="(bts, ii) in bt" class=" flex justify-start items-center flex-wrap ">
                        <template v-for="ib in bts">
                            <div class="p-1" v-if="getIndex(chat.opt?.buttons, ib) > -1">
                                <NButton @click="subV2(ib)" size="small" :type="ii == 1 ? 'warning' : 'primary'">{{
                                    getIndexName(chat.opt?.buttons, ib) }} </NButton>
                            </div>
                        </template>
                    </div>
                </template>
                ...
                <template v-else>
                    ...
                    <div class="flex space-x-2">
                        ...
                        <NButton type="warning" @click="sub('REROLL', 1)" size="small"
                            v-if="chat.opt?.action === 'IMAGINE'">{{ $t('mjchat.reroll') }}</NButton>

                    </div>
                </template>
            </div>
            <div v-else-if="!chat.loading">
                <NButton type="primary" @click="reload()">{{ $t('mjchat.reload') }}</NButton>
            </div>
            <div v-else-if="chat.opt.progress" class="py-2 min-w-[200px]">
                {{ $t('mjchat.progress') }}{{ chat.opt.progress }}</div>
        ...
        <NModal v-model:show="st.isShow" preset="card" :title="$t('mjchat.redrawEditing')" style="max-width: 800px;"
            @close="st.isShow = false">
            <aiCanvas :chat="chat" :base64="st.uri_base64" v-if="st.isShow" @success="maskOk" />
        </NModal>
        <NModal v-model:show="st.isCustom" preset="card" :title="$t('mj.customTitle')" style="max-width: 600px;"
            @close="st.isCustom = false">
    ...
    <div v-else class="w-[200px] h-[150px] flex flex-col justify-center items-center">
        ...
        <NButton type="primary" v-if="chat.opt?.imageUrl"><a :href="mjImgUrl(chat.opt?.imageUrl)" target="_blank">{{
            $t('mjchat.openurl') }}</a></NButton>
    </div>
</template>

<style>
.markdown-body img.maxCss,
img.maxCss,
.maxCss img {
