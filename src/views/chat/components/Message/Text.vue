...
<template>
    <div class="" :class="wrapClass">
        <div ref="textRef" class="leading-relaxed break-words" style="color:blue">
            <div v-if="!inversion">
                <aiTextSetting v-if="!inversion && isApikeyError(text)" />
                <aiSetAuth v-if="!inversion && isAuthSessionError(text)" />

                <dallText :chat="chat" v-if="chat.model == 'dall-e-3' || chat.model == 'dall-e-2'"
                ...
                <template v-else>
                    <div v-if="!asRawText" class="markdown-body" :class="{ 'markdown-body-generate': loading }"
                        v-html="text" />
            ...
            <div v-else class="markdown-body" style="--color-fg-default:#24292f;color:#fff" v-html="text" />
            <!-- <div v-else class="whitespace-pre-wrap" v-text="text" /> -->
            <MjTextAttr :image="chat.opt?.images[0]" v-if="chat.opt?.images"></MjTextAttr>
            <whisperText v-if="chat.model && chat.model.indexOf('whisper') > -1 && chat.opt?.lkey" :isW="true"
                :chat="chat" class="w-full" />
            <ttsText v-if="!inversion && chat.opt?.duration && chat.opt?.duration > 0 && chat.opt?.lkey" :isW="true"
                :chat="chat" class="w-full" />
        </div>
    </div>
</template>
