...
<template>
    <div class="p-2">
        ...
        <div class="pt-2 flex justify-center items-center w-full relative">
            ...
            <div class="absolute right-1 top-3 z-40" v-if="st.imgSrc">
                <NButton strong secondary round size="small" type="success" @click="clear(2)">{{ $t('common.clear') }}
                </NButton>
            </div>
            <div class="h-[180px] w-full overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
                @click=" fsRef.click()">
        ...
        <div class="pt-2 " v-if="useTem">
            <div class="relative h-[180px]">
                <div class="absolute right-1 top-1 z-40">
                    <NButton strong secondary round size="small" type="success" @click="clear(1)">{{ $t('common.clear')
                        }}
                    </NButton>
                </div>
                <div class=" absolute w-full h-full top-0 left-0 z-1">
                    ...
                    <video controls playsinline loop :poster="useTem.processedCoverURL"
                        class="  rounded-lg bg-[#242424] object-contain w-full h-full transition-all"
                        :src="useTem.processedURL"></video>
                </div>
            </div>
        </div>
        <div class="pt-2 " v-else-if="st.vgSrc">
            <div class="relative h-[180px]">
                <div class="absolute right-1 top-1 z-40">
                    <NButton strong secondary round size="small" type="success" @click="clear(3)">{{ $t('common.clear')
                        }}
                    </NButton>
                </div>
                <div class=" absolute w-full h-full top-0 left-0 z-1">
        ...
        <div class="pt-2 flex justify-center items-center w-full" v-else>
            ...
            <div
                class="h-[180px] w-full overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center ">
                <!-- <img :src="luma.image_url" v-if="luma.image_url" /> -->
                <div class="text-center">
                    <div class="cursor-pointer p-2" @click="selectVideo">{{ $t('dance.upvideo') }} </div>
                    <NButton type="primary" size="small" @click="st.showImg = true">{{ $t('dance.usevideo') }}
                    </NButton>
                </div>
            </div>
        </div>
    ...
    <n-drawer v-model:show="st.showImg" :placement="isMobile ? 'bottom' : 'right'"
        :class="isMobile ? ['!h-[90vh]'] : ['!w-[80vw]']" style="--n-body-padding:0">
        <n-drawer-content class="mydrawer" :closable="isMobile">
            <template #header>
                <div class="flex justify-between items-center w-full">
                    <div class="pr-4">{{ $t('dance.moban') }}</div>
                    <div class=" max-w-[400px]">
                        <n-input round :placeholder="t('dance.moban2')" clearable v-model:value="st.q"
