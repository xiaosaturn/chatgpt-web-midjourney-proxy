    ...
    <div class="p-2">
        <n-tabs type="segment" animated v-model:value="st.type">
            ...
            <n-tab-pane name="custom" :tab="$t('suno.custom')">
                ...
                <div class="pt-1">
                    <n-input v-model:value="cs.prompt" :disabled="des.make_instrumental" :placeholder="$t('suno.lypls')"
                        type="textarea" size="small" :autosize="{ minRows: 3, maxRows: 12 }" />
                </div>
                <div class="pt-1">
                ...
                <template v-if="cs.continue_clip_id && exSuno">
                    <div class="pt-5">
                        <div class="flex justify-between pb-3">
                            ...
                            <NTag type="success" size="small" round><span class="cursor-pointer"
                                    @click="cs.continue_clip_id = ''">清除</span></NTag>
                    ...
                    <div class="pt-1">
                        <div
                            ...
                            <div class="flex-1  pl-2">
                                ...
                                <div class="opacity-60 line-clamp-1 w-full text-[12px] cursor-pointer"
                                    v-if="exSuno.metadata && exSuno.metadata.prompt">
                                ...
                                <div class="text-right text-[14px] flex justify-end items-center space-x-2  ">
                                    ...
                                    <div v-if="exSuno.status == 'error'"
                                        class="text-[8px] flex items-center border-[1px] border-red-500/80 px-1 list-none rounded-md ">
                                        失败</div>
                                    <template v-if="exSuno.metadata && exSuno.metadata.duration">
                                        <div
                                            ...
                                            {{ exSuno.metadata.duration.toFixed(1) }}s</div>
                                    </template>
                                    <div class="text-[8px] flex items-center border-[1px] border-gray-500/30 px-1 list-none rounded-md"
                                        v-if="exSuno.major_model_version"> {{ exSuno.major_model_version }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="pt-4">
            <div class="flex justify-between items-start">
                <div class=" space-x-1">
                    <NTag v-if="st.type == 'custom'" type="success" size="small" round><span class="cursor-pointer"
                            @click="generateLyrics()">{{ $t('suno.generately') }}</span></NTag>
